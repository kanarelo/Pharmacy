	...
	<div class="row clearfix">
		<div class="col_12">
			<div class="widget clearfix" style="min-height: 300px;">
				<h2>Outpatient Details: {$patient->name}</h2>
				<div class="widget_inside drugs-search">
					...
					{else}
						...
						<br />
					{/if}
					{if $request->user->belongsToGroups('pharmacists,doctors')}
						<br />
					{/if}
					{if $request->user->belongsToGroups('pharmacists,doctors')}
						<a style="float: left" class="button" href="{#BASE_URL#}/patients/outpatients/{$patient->id}/assign-drug/">Assign {if $patient_products} More {/if}Drugs</a>
					{/if}
					{if $request->user->belongsToGroups('pharmacists')}
						<a style="float: right" class="button blue" href="{#BASE_URL#}/patients/outpatients/{$patient->id}/clear-bill/">Clear bill</a>
					{/if}
				</div>
			</div>
		</div>
